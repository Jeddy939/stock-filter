...
Invoke-Gcloud @("run", "jobs", "deploy", "moneymaker-fetch", "--image", $Image, "--region", $Region, "--command", "python", "--args=-m,firebase.worker", "--memory=4Gi", "--cpu=2", "--task-timeout=14400s", "--max-retries=0", "--set-cloudsql-instances", "$Project`:$Region`:moneymaker-db", "--set-env-vars", "MONEYMAKER_JOB_TYPE=fetch,MONEYMAKER_CACHE_BUCKET=$Bucket,GOOGLE_CLOUD_PROJECT=$Project", "--set-secrets", "MONEYMAKER_DATABASE_URL=moneymaker-database-url:latest")
Invoke-Gcloud @("run", "jobs", "deploy", "moneymaker-filter", "--image", $Image, "--region", $Region, "--command", "python", "--args=-m,firebase.worker", "--memory=4Gi", "--task-timeout=3600s", "--max-retries=0", "--set-cloudsql-instances", "$Project`:$Region`:moneymaker-db", "--set-env-vars", "MONEYMAKER_JOB_TYPE=filter,MONEYMAKER_CACHE_BUCKET=$Bucket,GOOGLE_CLOUD_PROJECT=$Project", "--set-secrets", "MONEYMAKER_DATABASE_URL=moneymaker-database-url:latest")
Invoke-Gcloud @("run", "jobs", "deploy", "moneymaker-import", "--image", $Image, "--region", $Region, "--command", "python", "--args=-m,firebase.worker", "--memory=4Gi", "--task-timeout=14400s", "--max-retries=0", "--set-cloudsql-instances", "$Project`:$Region`:moneymaker-db", "--set-env-vars", "MONEYMAKER_JOB_TYPE=import-sqlite,MONEYMAKER_CACHE_BUCKET=$Bucket,FIREBASE_STORAGE_BUCKET=$Bucket,MONEYMAKER_STORAGE_BUCKET=$Bucket,GOOGLE_CLOUD_PROJECT=$Project", "--set-secrets", "MONEYMAKER_DATABASE_URL=moneymaker-database-url:latest")
Invoke-Gcloud @("run", "jobs", "deploy", "moneymaker-export", "--image", $Image, "--region", $Region, "--command", "python", "--args=-m,firebase.worker", "--memory=1Gi", "--task-timeout=3600s", "--max-retries=0", "--set-cloudsql-instances", "$Project`:$Region`:moneymaker-db", "--set-env-vars", "MONEYMAKER_JOB_TYPE=export-ratings,MONEYMAKER_CACHE_BUCKET=$Bucket,FIREBASE_STORAGE_BUCKET=$Bucket,MONEYMAKER_STORAGE_BUCKET=$Bucket,GOOGLE_CLOUD_PROJECT=$Project", "--set-secrets", "MONEYMAKER_DATABASE_URL=moneymaker-database-url:latest")
Invoke-Gcloud @("run", "jobs", "deploy", "moneymaker-rating-outcomes", "--image", $Image, "--region", $Region, "--command", "python", "--args=-m,firebase.worker", "--memory=1Gi", "--task-timeout=3600s", "--max-retries=0", "--set-cloudsql-instances", "$Project`:$Region`:moneymaker-db", "--set-env-vars", "MONEYMAKER_JOB_TYPE=rating-outcomes,MONEYMAKER_CACHE_BUCKET=$Bucket,FIREBASE_STORAGE_BUCKET=$Bucket,MONEYMAKER_STORAGE_BUCKET=$Bucket,GOOGLE_CLOUD_PROJECT=$Project", "--set-secrets", "MONEYMAKER_DATABASE_URL=moneymaker-database-url:latest")
...
if ($ScheduleUpdates) {
    ...
    $asxArgs = @("scheduler", "jobs", "update", "http", "moneymaker-daily-fetch-asx", "--location=$Region",
        "--schedule=30 6 * * 1-5", "--time-zone=Australia/Brisbane", "--uri=$ApiUrl/api/scheduled-fetch",
