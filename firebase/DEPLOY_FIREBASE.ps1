...
Invoke-Gcloud @("run", "jobs", "deploy", "moneymaker-import", "--image", $Image, "--region", $Region, "--command", "python", "--args=-m,firebase.worker", "--memory=4Gi", "--task-timeout=14400s", "--max-retries=0", "--set-env-vars", "MONEYMAKER_JOB_TYPE=import-sqlite,MONEYMAKER_CACHE_BUCKET=$Bucket,FIREBASE_STORAGE_BUCKET=$Project.firebasestorage.app,MONEYMAKER_STORAGE_BUCKET=$Project.firebasestorage.app,GOOGLE_CLOUD_PROJECT=$Project", "--set-secrets", "MONEYMAKER_DATABASE_URL=moneymaker-database-url:latest")
Invoke-Gcloud @("run", "jobs", "deploy", "moneymaker-export", "--image", $Image, "--region", $Region, "--command", "python", "--args=-m,firebase.worker", "--memory=1Gi", "--task-timeout=3600s", "--max-retries=0", "--set-env-vars", "MONEYMAKER_JOB_TYPE=export-ratings,MONEYMAKER_CACHE_BUCKET=$Bucket,FIREBASE_STORAGE_BUCKET=$Project.firebasestorage.app,MONEYMAKER_STORAGE_BUCKET=$Project.firebasestorage.app,GOOGLE_CLOUD_PROJECT=$Project", "--set-secrets", "MONEYMAKER_DATABASE_URL=moneymaker-database-url:latest")
