...
class MoneymakerProAlphaApp:
    def __init__(self, root):
        ...
        sv_ttk.set_theme("dark") if sv_ttk else None
    ...
    def browse_ticker_file(self):
        filename = filedialog.askopenfilename(title="Select Ticker File", filetypes=(("Text files", "*.txt"), ("All files", "*.*")))
        if filename:
    ...
    def start_fetch(self):
        ...
        ticker_path = self.ticker_file_var.get().strip()
        if not ticker_path:
            messagebox.showerror("Error", "Please select a ticker file.")
            return

        # Prepare command
        fetcher = str(_get_fetcher_path())
        if not _Path(fetcher).exists():
        ...
        command = [sys.executable, "-u", fetcher, ticker_path, "-y", str(self.years_var.get()), "-w", str(self.workers_var.get()), "-o", self.output_file_var.get()]
        threading.Thread(target=self.run_process, args=(command,), daemon=True).start()


    def run_process(self, command):
        try:
            process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0), cwd=str(_get_fetcher_path().parent))
    ...
    def load_data_file(self):
        filename = filedialog.askopenfilename(title="Select Stock Data File", filetypes=(("JSON files", "*.json"), ("All files", "*.*")))
    ...
    def start_filter(self):
        ...
        try:
            ...
            t = threading.Thread(target=run_filter_thread, args=(config, self.stock_data, self.results_queue, self.progress_queue), daemon=True)
            t.start()
            self.filter_thread = t
