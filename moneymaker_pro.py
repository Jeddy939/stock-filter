...
class MoneymakerProApp:
    def __init__(self, root):
        self.root = root
        root.title("Moneymaker Pro - All-in-One")
    ...
    def browse_ticker_file(self):
        filename = filedialog.askopenfilename(title="Select Ticker File", filetypes=(("Text files", "*.txt"), ("All files", "*.* wurden")))
        if filename:
    ...
    def _create_filter_widgets(self, parent):
        ...
        self.run_filter_button = ttk.Button(controls_frame, text="Apply Filter", command=self.start_filter, style="Accent.TButton")
        self.run_filter_button.pack(fill=tk.X, pady=20)
        ...
        self.status_var = tk.StringVar(value="Ready. Load a data file to begin.")
    ...
    def load_data_file(self):
        filename = filedialog.askopenfilename(title="Select Stock Data File", filetypes=(("JSON files", "*.json"), ("All files", "*.* wurden")))
    ...
    def on_tree_click(self, event):
        if self.tree.identify_region(event.x, event.y) == "cell" and self.tree.identify_column(event.x) == "#1":
            ...
            if item_id: webbrowser.open_new_tab(f"https://finance.yahoo.com/quote/{self.tree.item(item_id, 'values')[0]}")
